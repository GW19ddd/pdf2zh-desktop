...
class BingTranslator(BaseTranslator):
    ...
    def find_sid(self):
        response = self.session.get(self.endpoint, timeout=(10, 60))
        response.raise_for_status()
        url = response.url[:-10]
...
class AnythingLLMTranslator(BaseTranslator):
    ...
    def do_translate(self, text):
        ...
        response = requests.post(
            self.api_url, headers=self.headers, data=json.dumps(payload), timeout=(10, 60)
        )
        response.raise_for_status()
        data = response.json()
...
class DifyTranslator(BaseTranslator):
    ...
    def do_translate(self, text):
        ...
        # 向 Dify 服务器发送请求
        response = requests.post(
            self.api_url, headers=headers, data=json.dumps(payload), timeout=(10, 60)
        )
        response.raise_for_status()
